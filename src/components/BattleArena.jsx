...
function lineSummary(idx, p1, p2, r, roll1, roll2) {
  const lead = r.p1.total + roll1*0.1 >= r.p2.total + roll2*0.1 ? 'P1' : 'P2';
  ...
  const lines = [
    ...
    `${luckNote}`,
    `Soul Dice → P1 d20=${roll1} · P2 d20=${roll2}. Score updated: P1 ${p1.wins} — P2 ${p2.wins}.`,
  ];
  return lines;
}
...
export default function BattleArena({ p1, p2, onComplete }) {
  ...
  const handleRoll = (who) => {
    setRolls((R) => ({ ...R, [who]: Math.ceil(Math.random()*20) }));
  };
  ...
  const resolveRound = () => {
    ...
    const p1Total = r1.total + rolls.p1 * 0.1 + ((Math.random()*2 - 1) * RANDOMNESS_RANGE * r1.total);
    const p2Total = r2.total + rolls.p2 * 0.1 + ((Math.random()*2 - 1) * RANDOMNESS_RANGE * r2.total);
        ...
        {[prepared.p1, prepared.p2].map((p, idx) => (
          <div key={idx} className="rounded-xl p-4 bg-white/5 border border-white/10 text-white">
            ...
            <div className="mt-3 flex items-center gap-2">
              <button onClick={()=>handleRoll(idx===0?'p1':'p2')} className="px-3 py-2 rounded bg-fuchsia-600 hover:bg-fuchsia-500 font-bold">Roll d20</button>
